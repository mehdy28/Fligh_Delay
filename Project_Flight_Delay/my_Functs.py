...
def null_vals_prct(data):
    ...
    return(Nan_values)
def null_vals_prct_plot(data):
    import matplotlib.pyplot as plt
    import seaborn as sns
        #Plotting
    plt.figure(figsize=(8,8))
    plt.title('Features with Highest NaN values Train data', size = 20)
    sns.barplot(x = null_vals_prct(data).NaN_Prcnt, y = null_vals_prct(data).Features, palette = 'rainbow')
    plt.xlabel("Percentage of NaN values in Categorical Features", size = 16)
    plt.ylabel("Features", size = 16)
    plt.show()
def null_vals_cols_del(data, prct):
    data.drop([null_vals_prct(data) >= prct], axis=1, inplace=True)
